...
function askGeminiAI(sheet, temp, hum, soil) {
  // 1. ดึง API Key จากตู้เซฟ
  var apiKey = PropertiesService.getScriptProperties().getProperty("GEMINI_API_KEY");
  if(apiKey) apiKey = apiKey.trim();
  
  // 2. ใช้ชื่อโมเดล gemini-2.5-flash
  var url = "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=" + apiKey;
  
  // 3. สร้าง Prompt สั่งงาน AI
  ...
  // 4. จัดรูปแบบข้อมูลเพื่อส่งให้ Gemini
  var payload = {
    "contents": [{
      "parts": [{"text": prompt}]
    }]
  ...
  try {
    // 5. ส่งข้อมูลไปหา AI และรอรับคำตอบ
    var response = UrlFetchApp.fetch(url, options);
    var json = JSON.parse(response.getContentText());
    
    // เช็ค Error จากเซิร์ฟเวอร์
    if (json.error) {
      sheet.getRange("I4").setValue("API ฟ้องว่า: " + json.error.message);
      sheet.getRange("I4").setBackground("#ea9999");
      return; 
    }
    
    // ดึงข้อความมาโชว์บน Google Sheets
    if (json.candidates && json.candidates.length > 0) {
      ...
      sheet.getRange("I4").setValue("เซิร์ฟเวอร์ AI ไม่ส่งคำตอบกลับมา");
    }
    
  } catch (e) {
    sheet.getRange("I4").setValue("โค้ด Error: " + e.message);
  }
}
